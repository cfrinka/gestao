...
async function buildAggregateFromLegacyCollections(month: string): Promise<{
  ...
  const [ordersSnap, paidBillsSnap, stockPurchasesSnap, exchangesSnap] = await Promise.all([
    adminDb.collection("orders").where("createdAt", ">=", startTs).where("createdAt", "<=", endTs).get(),
    adminDb.collection("bills").where("status", "==", "PAID").where("paidAt", ">=", startTs).where("paidAt", "<=", endTs).get(),
    adminDb.collection("stockPurchases").where("createdAt", ">=", startTs).where("createdAt", "<=", endTs).get(),
    adminDb.collection("exchanges").where("createdAt", ">=", startTs).where("createdAt", "<=", endTs).get(),
  ]);
...
function buildAggregateFromMovements(month: string, movements: FinancialMovementDoc[]): MonthAggregate {
  ...
  for (const movement of movements) {
    ...
    if (movement.type === "COGS") aggregate.cogs += amount;
    if (movement.type === "OPERATING_EXPENSE") aggregate.expenses += amount;
    if (movement.type === "STOCK_PURCHASE") aggregate.stockPurchasesCost += amount;
...
async function buildMonthAggregateWithDebug(month: string): Promise<{ aggregate: MonthAggregate; debug: MonthDebugRow }> {
  ...
  if (closureSnap.exists) {
    ...
    return {
      ...
      debug: {
        ...
        closureExists: true,
        movementCount: 0,
        legacyDiagnostics: { ordersCount: 0, paidBillsCount: 0, stockPurchasesCount: 0, exchangesCount: 0 },
        totals: {
  ...
  const movementsSnapshot = await adminDb.collection("financialMovements").where("competencyMonth", "==", month).get();
...
async function buildFiadoAging(endDate: Date) {
  ...
  const snapshot = await adminDb.collection("financialMovements").where("occurredAt", "<=", Timestamp.fromDate(endDate)).get();
...
async function buildProductMarginRanking(start: Date, end: Date) {
  ...
  const totalStock = sum(Array.from(productsById.values()).map((p) => Number(p.stock || 0)));
  const inventoryValue = sum(Array.from(productsById.values()).map((p) => Number(p.stock || 0) * Number(p.costPrice || 0)));
  const inventorySaleValue = sum(Array.from(productsById.values()).map((p) => Number(p.stock || 0) * Number(p.salePrice || 0)));
...
export async function generateReports(input: {
  ...
  const cacheKey = buildReportCacheKey(input.role, start, end, input.debugMode);
  const cacheRef = adminDb.collection("reportCache").doc(cacheKey);
  const cacheSnapshot = await cacheRef.get();

  if (!input.forceRefresh && cacheSnapshot.exists) {
    ...
    if (generatedAt && ttlMs > 0 && payload) {
      ...
      if (isFresh) {
        return { payload: { ...payload, cache: { hit: true, generatedAt: generatedAt.toISOString(), ttlMs } } };
      }
    }
  }
